...
header-includes:
  ...
  - "\\renewenvironment*{quote}{\\origquote\\itshape}{\\origendquote}"
include-before:
...
Åldersspannet sträcker sig från `r min(af$age,na.rm=TRUE)` till `r max(af$age,na.rm=TRUE)`. Medelåldern är `r round(mean(af$age, na.rm=TRUE),1)` och medianåldern `r median(af$age, na.rm=TRUE)`; fördelningen är positivt skev. Första kvartilen *Q₁*=`r quantile(af$age,prob=.25,na.rm=TRUE)` och *Q₃*=`r quantile(af$age,prob=.75,na.rm=TRUE)`; hälften av personerna återfinns i det åldersspannet. Se [@fig:age.boxplot].

```{r fig:age.boxplot, fig.cap="Lådagram av ålder. {#fig:age.boxplot}", echo=FALSE}
