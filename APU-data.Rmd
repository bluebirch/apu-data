...
Detta ger en åldersfördelning som speglar fördelningen av födelseår, vilket förefaller rimligt. Att den yngsta testade personen skulle vara 14 år är däremot orimligt eftersom man måste vara minst 16 år för att vara inskriven vid Arbetsförmedlingen. Detta gäller testperson `r af$tp[x]` där födelsedatum är `r af$birthdate[x]` och testdatum `r af$date[x]` (`ats.date`=`r af$ats.date[x]` och `hts.date`=`r af$hts.date[x]`). Födelsedatum kommer från `ats.birthyr` (`r af$ats.birthyr[x]`) som alltså än en gång framstår som otillförlitligt.

```{r fig.cap="Fördelning av ålder.", echo=FALSE}
...
Kön finns i `ats.sex` och `vts.sex`. Om man ställer variablerna mot varandra i en fyrfältstabell framgår att ett antal personer är kodade som män i det ena systemet och kvinnor i det andra. Detta är uppenbara felregistreringar.
...
n.female <- length(na.omit(af$gender[af$gender=='Man']))
n.na <- length(af$gender[is.na(af$gender)])
```


Datafilen innehåller då `r length(na.omit(af$gender[af$gender=='Man']))` män (`r round(length(na.omit(af$gender[af$gender=='Man']))/nrow(af)*100)`%), `r length(na.omit(af$gender[af$gender=='Kvinna']))` kvinnor (`r round(length(na.omit(af$gender[af$gender=='Kvinna']))/nrow(af)*100)`%) och `r length(af$gender[is.na(af$gender)])` okända (`r round(length(af$gender[is.na(af$gender)])/nrow(af)*100)`%). -- men återigen, detta handlar om juridiskt kön och frågan är om detta är meningsfullt alls.

42 % kvinnor och 58 % män
